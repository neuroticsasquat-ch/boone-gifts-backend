...
# ---------------------------------------------------------------------------
# can_view_list  (owner OR share OR a share to an occasion of one of the viewer's families)
# ---------------------------------------------------------------------------
...
def test_can_view_list_with_share_returns_true():
    with patch(f"{ACCESS}.find_share", return_value=object()), \
         patch(f"{ACCESS}.list_shared_to_any_occasion_of", return_value=False):
        assert access.can_view_list(None, _user(2), _list(5, owner_id=1)) is True


def test_can_view_list_with_occasion_share_returns_true():
    with patch(f"{ACCESS}.find_share", return_value=None), \
         patch(f"{ACCESS}.list_shared_to_any_occasion_of", return_value=True):
        assert access.can_view_list(None, _user(2), _list(5, owner_id=1)) is True


def test_can_view_list_shared_family_without_a_share_returns_false():
    # Co-membership alone is no longer visibility: the owner must have shared
    # this list to one of the family's occasions. can_view_list must not consult
    # co-membership.
    with patch(f"{ACCESS}.find_share", return_value=None), \
         patch(f"{ACCESS}.list_shared_to_any_occasion_of", return_value=False), \
...
def test_can_view_list_connection_only_returns_false():
    # No share, no occasion share. A connection may exist, but can_view_list must
